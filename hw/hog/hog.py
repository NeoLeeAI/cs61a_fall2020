...
def swine_align(player_score, opponent_score):
    ...
    if player_score and opponent_score:
        n = 2
        max_gcd = player_score if player_score < opponent_score else opponent_score
        while n <= max_gcd:
            if player_score % n == 0 and opponent_score % n == 0 and n >= 10:
                return True
            n += 1
        return False
    else:
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        def process(score):
                running_high_new1 = running_high # avoid error
                last_score_new1 = score
                change = score - last_score
                if change > running_high_new1:
                    running_high_new1 = change
                    print(running_high_new1, 'point(s)! The most yet for Player', who)
                return last_score_new1, running_high_new1
        if who == 0:
            last_score_new2, running_high_new2 = process(score0)
            return(announce_highest(who, last_score_new2, running_high_new2))
        else:
            last_score_new2, running_high_new2 = process(score1)
            return(announce_highest(who, last_score_new2, running_high_new2))
    return say
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    max_average = 0
    while n <= 10:
        average = make_averaged(roll_dice, trials_count)(n, dice)
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    bacon_socre = free_bacon(opponent_score)
    if extra_turn(score + bacon_socre, opponent_score):
        return 0
    else:
