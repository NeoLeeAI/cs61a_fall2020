def couple(s, t):
    ...
    assert len(s) == len(t)
    "*** YOUR CODE HERE ***"
    compled_list = [[s[i], t[i]] for i in range(len(s))]
    return compled_list
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    goal = make_city('goal', lat, lon)
    if distance(goal, city_a) <= distance(goal, city_b):
...
def sprout_leaves(t, leaves):
    ...
    if is_leaf(t):
        ...
        new_branch = [sprout_leaves(b, leaves) for b in branches(t)]
...
def riffle(deck):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [deck[k//2] if k%2 == 0 else deck[(len(deck)+k)//2] for k in range(len(deck))]
